...
async def get_objava(id: int):
    for objava in objave:
        if objava["id"] == id:
            return objava
    raise HTTPException(status_code=404, detail="Objava nije pronadjena")


@app.post("/korisnici/{korisnik}/objave", response_model=list[ObjavaResponse])
async def get_objave_korisnika(korisnik: str, auth: AuthRequest):
    async with aiohttp.ClientSession() as session:
        async with session.post(
            ...
            if not result.get("uspjesno", False):
                raise HTTPException(status_code=401, detail="Neispravni korisnicki podaci")
